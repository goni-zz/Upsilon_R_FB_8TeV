...
void doFitUpsilon_Data_AllParmFree_pa_y_3S_split10_4(
       int collId = kPADATA,  
       float ptLow=0, float ptHigh=30, 
       float yLow=-0.8, float yHigh=-0.4,
       ...
       )
{
  ...
  float yLow_Lab;
  float yHigh_Lab;
  if ( collId == kPPDATA) f1 = new TFile("../../skimmedFiles/");
  
  if ( collId == kPADATA) 
  {
    //f1 = new TFile("../../skimmedFiles/PA_8TeV_Pbp_skimmed_Final.root");
    //f2 = new TFile("../../skimmedFiles/PA_8TeV_pPb_skimmed_Final.root");
    //f1 = new TFile("../../skimmedFiles/Pbp_test_Trig-L1DoubleMu0_OpSign_20186202346_unIdentified.root");
    //f2 = new TFile("../../skimmedFiles/pPb_test_Trig-L1DoubleMu0_OpSign_20186202346_unIdentified.root");
    f1 = new TFile("../../skimmedFiles/PA_8TeV_Pbp_skimmed_L1DoubleMu0_SoftId.root");
    f2 = new TFile("../../skimmedFiles/PA_8TeV_pPb_skimmed_L1DoubleMu0_SoftId.root");
    yLow_Lab=yLow+0.47;
    yHigh_Lab=yHigh+0.47;
  }
 
  float eta_low = -2.4;
  float eta_high = 2.4;
  TString kineLabel = getKineLabel (collId, ptLow, ptHigh, yLow_Lab, yHigh_Lab, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High) ;
  TString kineLabel_out = getKineLabel (collId, ptLow, ptHigh, yLow, yHigh, muPtCut, cLow, cHigh, dphiEp2Low, dphiEp2High) ;
  //TString kineCut = Form("pt>%.2f && pt<%.2f && y>%.2f && y<%.2f",ptLow, ptHigh, yLow, yHigh);
  TString kineCut = Form("pt>%.2f && pt<%.2f && y>%.2f && y<%.2f && eta1<%.2f && eta1>%.2f && eta2<%.2f && eta2>%.2f",ptLow, ptHigh, yLow_Lab, yHigh_Lab, eta_high,eta_low, eta_high,eta_low );
  ...
  //float n_min = 1.081, n_max = 4.762;
  float n_min = 1.081, n_init = 3.9, n_max = 4.762;
  float a_min = 1.081, a_init = 1.3, a_max = 4.762;
  float s_min = 0.01, s_max = 0.3;
  float f_min = 0, f_max = 1;
  float x_min = 0, x_max = 2.5;
  float err_mu_max = 30, err_sig_max = 30, err_lamb_max = 30;

  RooRealVar    sigma1s_1("sigma1s_1","width/sigma of the signal gaussian mass PDF",0.05, s_min, s_max);
  ...
  RooRealVar *x1s = new RooRealVar("x1s","sigma ratio ", 0.5, x_min, x_max);
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", a_init, a_min, a_max);
  ...
  RooRealVar n1s_1("n1s_1","power order", n_init, n_min, n_max);
  ...
  RooRealVar *f1s = new RooRealVar("f1s","1S CB fraction", 0.5, f_min, f_max);
...
//  } 
  ...
  RooRealVar *nSig2s= new RooRealVar("nSig2s"," 2S signals",-20,500000);
  RooRealVar *nSig3s= new RooRealVar("nSig3s"," 3S signals",-30,250000);
  ...
  RooRealVar err_mu("#mu","err_mu",5,  0, err_mu_max) ;
  RooRealVar err_sigma("#sigma","err_sigma", 5, 0, err_sig_max);
  RooRealVar m_lambda("#lambda","m_lambda",  5, 0, err_lamb_max);
...
//if(yLow==0) drawText(Form("|y^{#mu#mu}| < %.1f",yHigh ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  ...
  if(collId != kPPDATA && collId != kPPMCUps1S && collId != kPPMCUps2S) 
  {
      ...
      drawText(Form("|#eta^{#mu}_{Lab}| < %.2f",eta_high), pos_text_x,pos_text_y-pos_y_diff*3,text_color,text_size);
  ...
  else if(collId == kPADATA) CMS_lumi(pad1, 31 ,33);
  ...
  TFile* outf = new TFile(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s.root",kineLabel_out.Data()),"recreate");
  outh->Write();
  c1->SaveAs(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s.pdf",kineLabel_out.Data()));
  c1->SaveAs(Form("AllParmFree_fitresults_upsilon_DoubleCB_5TeV_%s.png",kineLabel_out.Data()));
  c1->Write();
  ws->Write();

  cout << "N, alpha, sigma1s, M0, f, X double CB for data " << endl;
  cout << Form(" else if ( binMatched( %.f, %.f, %.f, %.1f, %.1f) ) {setSignalParMC(",muPtCut, ptLow, ptHigh, yLow, yHigh)<<endl;
  cout << " n: " << ws->var("n1s_1")->getVal() << "+-" << ws->var("n1s_1")->getError()<< ", \n";
  cout << " a: " << ws->var("alpha1s_1")->getVal() << "+-" << ws->var("alpha1s_1")->getError()<< ", \n";
  cout << " s: " << ws->var("sigma1s_1")->getVal() << "+-" << ws->var("sigma1s_1")->getError()<< ", \n";
  cout << " m: " << ws->var("m_{#Upsilon(1S)}")->getVal() << "+-" << ws->var("m_{#Upsilon(1S)}")->getError() << ", \n";
  cout << " f: " << ws->var("f1s")->getVal() << "+-" << ws->var("f1s")->getError() <<", \n";
  cout << " x: " << ws->var("x1s")->getVal() << "+-" << ws->var("x1s")->getError() << endl;

  const string output="params";
  gSystem->mkdir(Form("%s",output.c_str()),1);
  if(ws->var("n1s_1")->getVal()>ws->var("n1s_1")->getError()&&ws->var("alpha1s_1")->getVal()>ws->var("alpha1s_1")->getError()&&ws->var("sigma1s_1")->getVal()>ws->var("sigma1s_1")->getError()&&ws->var("m_{#Upsilon(1S)}")->getVal()>ws->var("m_{#Upsilon(1S)}")->getError()&&ws->var("f1s")->getVal()>ws->var("f1s")->getError()&&ws->var("x1s")->getVal()>ws->var("x1s")->getError()&&(ws->var("n1s_1")->getVal()-ws->var("n1s_1")->getError())>n_min&&(ws->var("n1s_1")->getVal()+ws->var("n1s_1")->getError())<n_max&&(ws->var("alpha1s_1")->getVal()-ws->var("alpha1s_1")->getError())>a_min&&(ws->var("alpha1s_1")->getVal()+ws->var("alpha1s_1")->getError())<a_max&&(ws->var("sigma1s_1")->getVal()-ws->var("sigma1s_1")->getError())>s_min&&(ws->var("sigma1s_1")->getVal()+ws->var("sigma1s_1")->getError())<s_max&&(ws->var("f1s")->getVal()-ws->var("f1s")->getError())>f_min&&(ws->var("f1s")->getVal()+ws->var("f1s")->getError())<f_max&&(ws->var("x1s")->getVal()-ws->var("x1s")->getError())>x_min&&(ws->var("x1s")->getVal()+ws->var("x1s")->getError())<x_max){
    ofstream of(Form("%s/pt%.1f_%.1f_y%.1f_%.1f.txt",output.c_str(),ptLow,ptHigh,yLow,yHigh));
    of << "N, alpha, sigma1s, M0, f, X double CB for data " << endl;
    of << Form(" else if ( binMatched( %.f, %.f, %.f, %.1f, %.1f) ) {setSignalParMC(",muPtCut, ptLow, ptHigh, yLow, yHigh)<< ws->var("n1s_1")->getVal() <<", "<< ws->var("alpha1s_1")->getVal() <<", "<< ws->var("sigma1s_1")->getVal() <<", "<< ws->var("m_{#Upsilon(1S)}")->getVal() <<", "<< ws->var("f1s")->getVal() <<", "<< ws->var("x1s")->getVal() <<"}"<< endl;
    cout<<"\n          ###### OK IT'S FINE :) ######"<<endl;
  }
  else if(ws->var("n1s_1")->getVal()<ws->var("n1s_1")->getError()||ws->var("alpha1s_1")->getVal()<ws->var("alpha1s_1")->getError()||ws->var("sigma1s_1")->getVal()<ws->var("sigma1s_1")->getError()||ws->var("m_{#Upsilon(1S)}")->getVal()<ws->var("m_{#Upsilon(1S)}")->getError()||ws->var("f1s")->getVal()<ws->var("f1s")->getError()||ws->var("x1s")->getVal()<ws->var("x1s")->getError()) cout<<"###ERROR is larger than VALUE### CHECK WTF is ;( ###"<<endl;
