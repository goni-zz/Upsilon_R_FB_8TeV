...
void doFitUpsilon_Data_AllParmFree_pa_y_3S_split10_1(
       int collId = kPADATA,  
       ...
       )
{
  ...
  RooRealVar alpha1s_1("alpha1s_1","tail shift", 2.1 , 1.2, 3.7);
  ...
  RooRealVar n1s_1("n1s_1","power order", 2.1 , 1.2, 3.3);
...
//if(yLow==0) drawText(Form("|y^{#mu#mu}| < %.1f",yHigh ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  drawText(Form("%.2f < y^{#mu#mu}_{CM} < %.2f",yLow_Lab,yHigh_Lab ), pos_text_x,pos_text_y-pos_y_diff,text_color,text_size);
  ...
  cout << ws->var("n1s_1")->getVal() << "+-" << ws->var("n1s_1")->getError()<< ", \n";
  cout << ws->var("alpha1s_1")->getVal() << "+-" << ws->var("alpha1s_1")->getError()<< ", \n";
  cout << ws->var("sigma1s_1")->getVal() << "+-" << ws->var("sigma1s_1")->getError()<< ", \n";
  cout << ws->var("m_{#Upsilon(1S)}")->getVal() << "+-" << ws->var("m_{#Upsilon(1S)}")->getError() << ", \n";
  cout << ws->var("f1s")->getVal() << "+-" << ws->var("f1s")->getError() <<", \n";
  cout << ws->var("x1s")->getVal() << "+-" << ws->var("x1s")->getError() <<" );}" << endl;
  ...
  else if(ws->var("n1s_1")->getVal()>ws->var("n1s_1")->getError()||ws->var("alpha1s_1")->getVal()>ws->var("alpha1s_1")->getError()||ws->var("sigma1s_1")->getVal()>ws->var("sigma1s_1")->getError()||ws->var("m_{#Upsilon(1S)}")->getVal()>ws->var("m_{#Upsilon(1S)}")->getError()||ws->var("f1s")->getVal()>ws->var("f1s")->getError()||ws->var("x1s")->getVal()>ws->var("x1s")->getError()){
